...
extension UIImageView {
    ...
    func downloadedFrom(url: URL, contentMode mode: UIView.ContentMode = .scaleAspectFit) {
        ...
        URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            DispatchQueue.main.async() {
                self.image = image
                if images.countImages >= dictionaries.dictoPokemon.count - 1{
                    NotificationCenter.default.post(name: NSNotification.Name(rawValue: "load"), object: nil)
                }
                
                images.image.image = self.image
                images.dictionary.append(["img": images.image])
                images.countImages += 1
            }
            }.resume()
    }
    ...
    func setRandomDownloadImage(urlString: String) {
        if self.image != nil {
            self.alpha = 1
            return
        }
        ...
        let url = URL(string: "http://lorempixel.com/80/80/")!
